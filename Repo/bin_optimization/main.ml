...
let output = get_ast_test "CMPNEQ"
...
let test_read_file () =
  let str = read_file "./src/bin_optimization/contracts/hardlimit.tz" in
  print_string str
...
let test_script () =
  ...
  run_script
    ctx
    (*(read_file "./contracts/hardlimit2.tz")*)
...
let  =
  let storage = "Unit" in
  let parameter = "Int" in
  let srcipt = read_file "./contracts/hardlimit.tz" in
