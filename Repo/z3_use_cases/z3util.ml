...
let add_soft_constraints =
  List.map
    ~f:
      (List.map ~f:(fun (constr, weight) ->
           add_soft_gas constr ([%show: int] weight)))
...
let show_smt enc enc_weights =
  ...
  let s = Z3.Optimize.to_string !octxt in
  Z3.Optimize.pop !octxt ;
  s
*)

let list_sort = Z3List.mk_sort !ctxt
...
let sort_list = Z3List.mk_list_s !ctxt
...
let stact_sort = Z3List.mk_list_s !ctxt "int_list" int_sort
...
let solve_max_model_exn enc enc_weights =
  ...
  let _ = Z3.Optimize.add !octxt [enc] in
  let mdl =
    match Z3.Optimize.check !octxt with
    | Solver.SATISFIABLE ->
        Option.value_exn
          (Z3.Optimize.get_model !octxt)
          ~message:"SAT but no model"
