...
let pp fmt node =
  ...
  let printable =
    Micheline_printer.printable Michelson_v1_primitives.string_of_prim canonical
  in
...
let rec parse_ty :
 ...
 fun ~allow_big_map ~allow_operation ~allow_contract node ->
  ...
  | _ ->
      ...
      Stdlib.failwith ("Michelson_value_sampler.parse_ty: could not parse " ^ s)
...
let michelson_samplers =
  let module Config = struct
    ...
    let parameters =
      { int_size = { min = 0; max = 8 };
        string_size = { min = 0; max = 8 };
        bytes_size = { min = 0; max = 8 };
        stack_size = { min = 0; max = 16 };
...
let gen_node_with_target_type global_type_string =
  string_input_to_exvalue global_type_string >>=? fun res ->
  let (exv, ctxt) = res in
  match exv with
  | Ex_Value (a_ty, a) ->
...
let print_prims global_type_string =
  gen_node_with_target_type global_type_string >>=? fun full_node ->
