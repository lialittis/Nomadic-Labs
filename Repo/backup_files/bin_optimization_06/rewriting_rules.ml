...
let mk_rules (module D : Dist) =
  ( module struct
    ...
    (*initial node : could be Ill-Typed *)
    let gen_init_node (filename : string) =
      ...
      let full_node = M.mutate_2 initial_node 0 ~max_loop:100 in
      full_node
    ...
    let process (filename : string) (relationships_file : string)
        initial_global_gas =
      (* given an argument of distance bound for ill-typed node *)
      let max_distance = 10000000. in
      let json = Yojson.Basic.from_file relationships_file in
      let samples = get_samples json in
      (*let nsample = Array.length samples in*)
      let outputs = of_outputs samples in

      (* running times for local search *)
      let max_loop = 20 in

      let gen_sol (full_node : M.full_node) outputs outputs' gas =
        let d = sum_dists D.dist outputs outputs' in
        Printf.printf
          "Haven the node %s , distance is %f\n"
          (to_string full_node.n)
          d ;
        return @@ { full_node; cost = gas; distance = d }
      in

      let gen_sol_ill (full_node : M.full_node) max_distance =
        Random.self_init () ;
        let d = Random.float max_distance (* temp *) in
        let gas = string_of_float (d /. 500.) in
        return { full_node; cost = gas; distance = d }
      in
      ...
      let perturbate outputs ctxt =
        ...
        match updated_node.st with
        | Well_Typed ->
            print_endline "START BY NODE : " ;
            print_node updated_node.n ;
            interprete_random_node
              samples
              updated_node.n
              ctxt
              initial_global_gas
        ...
        | Ill_Typed ->
            print_endline "SORRY TO  START BY ILL NODE : " ;
            print_node updated_node.n ;
            gen_sol_ill updated_node max_distance
      in
      ...
      let rec loop sol n ctxt =
        if n = max_loop || D.is_zero sol.distance then
          ...
          M.mutate_2 sol.full_node.n 0 ~max_loop:100 >>=? fun mutated ->
          match mutated.st with
          | Well_Typed ->
              interprete_random_node samples mutated.n ctxt initial_global_gas
              >>= fun (outputs', gas) ->
              gen_sol mutated outputs outputs' gas >>=? fun sol' ->
              if sol'.distance < sol.distance then (
                (* restart *)
                Printf.printf "------Take this node------\n" ;
                loop sol' 0 ctxt )
              else (
                (* cost one time *)
                Printf.printf
                  "-------Drop this node and back to the last one!------\n" ;
                loop sol (n + 1) ctxt )
          | Ill_Typed ->
              gen_sol_ill mutated max_distance >>=? fun sol' ->
              if sol'.distance < sol.distance then (
                (* restart *)
                Printf.printf "------Take this node------\n" ;
                loop sol' 0 ctxt )
