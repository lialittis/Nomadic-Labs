  ...
  let rec mutate (node : node) =
    ...
    | Seq (l, node_list) ->
        if l = 0 then
          ...
          return @@ Seq (l, n :: node_list)

  let rec mutate_2 ?additions:(add = 1) ~(max_loop : int) ~(max_mutation : int)
      (node : node) (n : int) =
    if add = max_loop then (
      print_endline
        "##########################################\n\
...
*)
      ...
      | Error _ ->
          if n > 0 then
            mutate_2 node (n - 1) ~additions:add ~max_loop ~max_mutation
          else
            mutate_2
              node
              max_mutation
