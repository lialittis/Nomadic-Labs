...
let intsymbol = Symbol.mk_int !ctxt

let stringsymbol = Symbol.mk_string !ctxt

let symbol_to_string = Symbol.to_string
...
let stack_sort = Z3List.mk_list_s !ctxt "int_list" int_sort

(* fun declaration*)

(* mk_decl *)
let mk_const_decl = FuncDecl.mk_const_decl_s !ctxt

(* list funs *)
let cons_fun = Z3List.get_cons_decl stack_sort

let get_hd_decl = Z3List.get_head_decl stack_sort

let get_tl_decl = Z3List.get_tail_decl stack_sort

let mk_cons = FuncDecl.apply cons_fun

let get_hd = FuncDecl.apply get_hd_decl

let get_tl = FuncDecl.apply get_tl_decl

let is_nil = FuncDecl.apply (Z3List.get_is_nil_decl stack_sort)
...
let int_with_value = Arithmetic.Integer.mk_numeral_i !ctxt
...
let mk_bin_op bv_op int_op x y =
  match
    (Sort.get_sort_kind (Expr.get_sort x), Sort.get_sort_kind (Expr.get_sort y))
  with
...
let ( <+> ) =
  mk_bin_op BitVector.mk_add (fun ctxt x y -> Arithmetic.mk_add ctxt [x; y])

let ( <-> ) =
  mk_bin_op BitVector.mk_sub (fun ctxt x y -> Arithmetic.mk_sub ctxt [x; y])

let ( <*> ) =
  mk_bin_op BitVector.mk_mul (fun ctxt x y -> Arithmetic.mk_mul ctxt [x; y])
...
let solve_model_timeout cs timeout =
  ...
  | Solver.SATISFIABLE ->
      ...
      Some
        (Option.value_exn (Solver.get_model slvr) ~message:"SAT but no model")
...
let get_objectives enc enc_weights =
  ...
  let s = conj (Z3.Optimize.get_objectives !octxt) in
